...
int bit_vec_filter_sse1(char* read, char* ref, int length, int max_error) {
  ...
  uint8_t read_vec0_t1[SSE_BYTE_NUM] __aligned__;
  uint8_t read_vec1_t1[SSE_BYTE_NUM] __aligned__;
  uint8_t ref_vec0_t1[SSE_BYTE_NUM] __aligned__;
  uint8_t ref_vec1_t1[SSE_BYTE_NUM] __aligned__;

  sse3_convert2bit1(read, read_vec0_t1, read_vec1_t1);
  sse3_convert2bit1(ref, ref_vec0_t1, ref_vec1_t1);
  ...
  return bit_vec_filter_m128_sse1(read_vec0_t1, read_vec1_t1,
      ref_vec0_t1, ref_vec1_t1, mask, max_error);
}
